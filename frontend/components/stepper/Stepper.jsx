			...
			<div className="flex justify-center">
				<ol className="relative mx-4 py-6 px-6 border-4 border-black rounded-3xl">
					{stepArray.map((el, index) => (
						<li className="mb-10 last:mb-0 flex flex-col md:flex-row items-start border-4 border-red-400" key={index}>
							...
							<div className="md:ml-6">
								<h3 className={"border-4 border-blue-400 text-3xl font-semibold text-black dark:text-white mt-1 text-center md:text-left" + (index <= currentStep && el.canGoBackTo ? " hover:cursor-pointer" : "")} onClick={index <= currentStep && el.canGoBackTo ? () => setCurrentStep(index) : null}>{el.title}</h3>
								<div className="my-6 w-[95vw] md:w-[60vw] border-4 border-purple-400">
								{index === currentStep ?
									<>
									{el.content}
									<div className="flex flex-row justify-center border-4 border-green-400">
										{el.showPrevious ?
